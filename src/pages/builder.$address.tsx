...
export const Route = createFileRoute('/builder/$address')({
...
function BuilderProfilePage() {
  ...
  const shipRate = profile.questsTotal > 0 ? Math.round((profile.questsShipped / profile.questsTotal) * 100) : 0
  const filteredQuests = tabFilter === 'all' ? mockQuests : mockQuests.filter((q) => q.status === tabFilter)
            ...
            <table className="w-full">
              ...
              <tbody>
                {filteredQuests.map((quest) => (
                  <tr key={quest.publicKey} className="border-b border-border/30 last:border-0 hover:bg-secondary/20 transition-colors">
                    <td className="p-4">
                      <Link to="/quest/$id" params={{ id: quest.publicKey }} className="text-sm font-medium text-foreground hover:text-primary transition-colors">
                        {quest.title}
                      </Link>
                    </td>
                    ...
                    <td className="p-4"><StatusBadge status={quest.status} /></td>
                    <td className="p-4 text-right">
                      <span className="text-sm font-mono font-bold text-primary">{lamportsToSol(quest.stakeAmount).toFixed(1)} SOL</span>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
            {filteredQuests.length === 0 && <div className="text-center py-8 text-muted-foreground text-sm">No quests found.</div>}
          </div>
        </div>
      </main>
      <Footer />
